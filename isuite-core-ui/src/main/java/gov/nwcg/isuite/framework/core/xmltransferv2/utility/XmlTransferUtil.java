package gov.nwcg.isuite.framework.core.xmltransferv2.utility;
...
public class XmlTransferUtil {
			...
			if(xf.ischardata==false){
				...
				switch(dtte.type)
				{
					case 0:  // string
						...
						if(i==0){
							if(StringUtility.hasValue(xf.derivedtable)){
								if(StringUtility.hasValue(val))
									fieldValues="(select id from "+xf.derivedtable+" where transferable_identity='" + val + "')";
								else
									fieldValues="null";
							}else{
								if(val!=null){
							...
							if(StringUtility.hasValue(xf.derivedtable)){
								if(StringUtility.hasValue(val))
									fieldValues=fieldValues+",(select min(id) from "+xf.derivedtable+" where transferable_identity='" + val + "')";
								else
	...
	public static String createUpdateSql(XmlTable xtTable, Object xmlObject, Boolean isOracle) throws Exception {
		...
		for(XmlField xf : xtTable.xmlFields){
			...
			if(xf.name.equalsIgnoreCase("TI")){
					...
					case 0:  // string
						...
						if(StringUtility.hasValue(xf.derivedtable)){
							if(StringUtility.hasValue(val)){
								fieldUpdates
									=fieldUpdates+
										"(select min(id) from "+xf.derivedtable+" where transferable_identity='" + val + "')";
							}else{
