package gov.nwcg.isuite.framework.customreports.designer;
...
public class DesignModifierDelegate {
	 ...
	 * @throws Exception
	 */
	public static JasperDesign modifyCustomReportDesign(IReport customReport, CustomReportVo customReportVo, JasperDesign jasperDesign) throws JRException, Exception{
		...
		JRDesignBand pageHeader = createPageHeader(customReport, customReportVo, jasperDesign);
...
//	    textField.setHeight(15);		// Caller should reset
	...
	private static JRDesignBand createPageHeader(IReport customReport, CustomReportVo customReportVo, JasperDesign jasperDesign){
		JRDesignFrame rowFrame = createFrameForHeaderRow(customReport, customReportVo);	  
		
		//If PDF: Use existing header (from JRXML) and append to it.
		//If Excel: Discard existing header. 
		JRDesignBand headerBand;
		int headerBandOriginalHeight;
		if(customReport.isExportToExcel()) {
			headerBand = new JRDesignBand();
			headerBandOriginalHeight = 0;
			rowFrame.setY(0);
		} else {
			headerBand = (JRDesignBand) jasperDesign.getPageHeader();
			headerBandOriginalHeight = headerBand.getHeight();
			rowFrame.setY(headerBandOriginalHeight+1);
		}
	    ...
	    headerBand.setHeight(rowFrame.getY()+rowFrame.getHeight());
	...
	private static void finalizeDesign(IReport customReport, CustomReportVo customReportVo, JasperDesign jasperDesign) throws JRException{
		...
		if(customReport.isExportToExcel()){ // REPORT TYPE: EXCEL
			...
			JRElement frameTitle = jasperDesign.getPageHeader().getElementByKey("frameTitle");
			JRDesignTextField txtFieldHeaderTitle = (JRDesignTextField)jasperDesign.getPageHeader().getElementByKey("txtFieldHeaderTitle");
			JRElement lineUnderTitle = jasperDesign.getPageHeader().getElementByKey("lineUnderTitle");
			JRDesignTextField txtFieldHeaderSubTitle = (JRDesignTextField)jasperDesign.getPageHeader().getElementByKey("txtFieldHeaderSubTitle");
			
			JRDesignTextField printTime = (JRDesignTextField)jasperDesign.getPageHeader().getElementByKey("printTime");
			
			int printTimeEndingPixel = printTime.getX()+printTime.getWidth();
			int shiftPrintDateTimeOffset = contentWidth - printTimeEndingPixel;
			printTime.setX(printTime.getX()+shiftPrintDateTimeOffset);
			...
			if(orientationEnum.equals(OrientationEnum.PORTRAIT)){
				JRElement lblCustomReport = jasperDesign.getPageHeader().getElementByKey("lblCustomReport");
				JRDesignTextField txtFieldIncidentNames = (JRDesignTextField)jasperDesign.getPageHeader().getElementByKey("txtFieldIncidentNames");
				txtFieldIncidentNames.setX(lblCustomReport.getWidth()+1);
				txtFieldIncidentNames.setWidth(printTime.getX()-txtFieldIncidentNames.getX()-1);
			}
			
			// Page Footer Properties
			JRDesignTextField pageNum = (JRDesignTextField)jasperDesign.getPageFooter().getElementByKey("pageNum");
			JRDesignTextField totalPages = (JRDesignTextField)jasperDesign.getPageFooter().getElementByKey("totalPages");
			JRElement lineAboveFooter = jasperDesign.getPageFooter().getElementByKey("lineAboveFooter");
			
			int totalPagesEndingPixel = totalPages.getX()+totalPages.getWidth();
			int shiftPagesOffset = contentWidth - totalPagesEndingPixel;
			pageNum.setX(pageNum.getX()+shiftPagesOffset);
			totalPages.setX(totalPages.getX()+shiftPagesOffset);
			
			lineAboveFooter.setWidth(contentWidth);
			
			// Last Page Footer Properties
			JRDesignTextField lastPageFooter_pageNum = (JRDesignTextField)jasperDesign.getLastPageFooter().getElementByKey("pageNum");
			JRDesignTextField lastPageFooter_totalPages = (JRDesignTextField)jasperDesign.getLastPageFooter().getElementByKey("totalPages");
			JRElement lastPageFooter_lineAboveFooter = jasperDesign.getLastPageFooter().getElementByKey("lineAboveFooter");
			
			lastPageFooter_pageNum.setX(pageNum.getX());
			lastPageFooter_totalPages.setX(totalPages.getX());
			lastPageFooter_lineAboveFooter.setWidth(lineAboveFooter.getWidth());
