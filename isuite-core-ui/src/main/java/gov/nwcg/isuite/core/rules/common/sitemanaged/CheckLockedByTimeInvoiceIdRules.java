...
public class CheckLockedByTimeInvoiceIdRules extends AbstractSiteManagedRule implements IRule{
	public static final String _RULE_NAME=SiteManagedRuleFactory.RuleEnum.IS_INCIDENT_LOCKED_BY_TIME_INVOICE_ID.name();
	
	public CheckLockedByTimeInvoiceIdRules(ApplicationContext ctx)
	{
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		if(LongUtility.hasValue(super.id) 
				&& super.idType.equals("TIMEINVOICE")
			...
			Long resourceId=tidao.getResourceId(super.id);
			if(LongUtility.hasValue(resourceId)){
				IncidentResourceDao irDao = (IncidentResourceDao)context.getBean("incidentResourceDao");
				Long incidentResourceId=irDao.getIdByResourceId(resourceId);
