...
public class IncidentRestrictRulesHandler extends AbstractRuleHandler {
	
	public IncidentRestrictRulesHandler(ApplicationContext ctx) {
		super.context=ctx;
	}
	...
	public int execute(Incident incidentEntity, IncidentVo vo, DialogueVo dialogueVo) throws Exception {
		...
		try{
			...
			for(RestrictIncidentRuleFactory.RuleEnum ruleEnum : RestrictIncidentRuleFactory.RuleEnum.values()){
				IRule rule = RestrictIncidentRuleFactory.getInstance(ruleEnum, context, vo, incidentEntity);
