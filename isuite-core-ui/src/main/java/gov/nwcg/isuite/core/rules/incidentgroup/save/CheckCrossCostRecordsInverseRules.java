...
public class CheckCrossCostRecordsInverseRules extends AbstractIncidentGroupSaveRule implements IRule {
	public static final String _RULE_NAME = IncidentGroupSaveRuleFactory.RuleEnum.CHECK_CROSS_COST_RECORDS_INVERSE.name();
		...
		try{
			if(null != entity){
				...
				for(Incident inc : removeList){
					...
					int cnt = super.incidentGroupDao.getCrossIncidentCostRecordsInverse(inc.getId());
					if(cnt > 0){
