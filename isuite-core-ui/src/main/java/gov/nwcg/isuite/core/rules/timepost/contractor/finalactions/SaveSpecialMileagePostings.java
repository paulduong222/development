...
import gov.nwcg.isuite.core.rules.timepost.contractor.ContractorSpecialMileageDuplicateRules;
...
public class SaveSpecialMileagePostings extends BaseFinalAction {
	private Boolean needOverwrite=false;
	private Collection<Long> duplicateIds = new ArrayList<Long>();
	
	public SaveSpecialMileagePostings(TimePostDao dao
	...
	public void save(AssignmentTimePost entity,DialogueVo dialogueVo) throws Exception {
		 ...
		 */
		needOverwrite = 
			checkForAdditionalActionNeeded(ContractorSpecialMileageDuplicateRules._RULE_NAME,dialogueVo);
		
		if(needOverwrite==true){
			/*
			 * if overwriting, update all duplicateIds
			 */
			CourseOfActionVo coaVo = super.getCoa(ContractorSpecialMileageDuplicateRules._RULE_NAME, dialogueVo);
			if(null != coaVo){
