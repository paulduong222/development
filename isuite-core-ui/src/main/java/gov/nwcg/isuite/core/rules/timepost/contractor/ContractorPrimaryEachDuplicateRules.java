...
/*
 * Rules to determine if contractor 'EACH' time posting is a duplicate
 * of another each time posting.
 */
public class ContractorPrimaryEachDuplicateRules extends AbstractContractorRule implements IRule {
	public static final String _RULE_NAME="CONTRACTOR_PRIMARY_EACH_DUPLICATE";

	public ContractorPrimaryEachDuplicateRules(ApplicationContext ctx){
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		...
		if((postType.equals("PRIMARY") || postType.equals("BOTH")) 
				&& getPrimaryUnitOfMeasure().equals("EACH")){
			...
			if(null != startDate && null != stopDate) {
				/*
				 * Query for any duplicate Primary Each posting dates.
				...
				if(CollectionUtility.hasValue(dupDates)){
					Collection<Long> dupIds = tpDao.getDuplicateDailyPostsIds(tpqFilter);
					...
					coaVo.setPromptVo(new PromptVo("text.time","action.0140a",new String[]{"Primary 'EACH'", "[ "+duplicateDates+" ]"},PromptVo._YES | PromptVo._NO | PromptVo._CANCEL));
