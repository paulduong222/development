...
import gov.nwcg.isuite.core.rules.iap.saveambulance.SaveAmbulanceRuleFactory;
...
public class IapSaveAmbulanceRulesHandler extends AbstractRuleHandler {

	public IapSaveAmbulanceRulesHandler(ApplicationContext ctx) {
		super.context = ctx;
	}
	...
	public int execute(IapMedicalAidVo vo, IapMedicalAid entity, DialogueVo dialogueVo) throws Exception {
		...
		try {
			...
			for(SaveAmbulanceRuleFactory.RuleEnum ruleEnum : SaveAmbulanceRuleFactory.RuleEnum.values()) {
				IRule rule = SaveAmbulanceRuleFactory.getInstance(ruleEnum, context, vo, entity);
