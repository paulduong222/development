...
/*
 * Rules to determine if contractor 'MILEAGE' special time posting is a duplicate
 * of another 'MILEAGE' time posting.
 */
public class ContractorSpecialMileageDuplicateRules extends AbstractContractorRule implements IRule {
	public static final String _RULE_NAME="CONTRACTOR_SPECIAL_MILEAGE_DUPLICATE";

	public ContractorSpecialMileageDuplicateRules(ApplicationContext ctx){
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		...
		if((postType.equals("SPECIAL") || postType.equals("BOTH"))
				&& getSpecialUnitOfMeasure().equals("MILEAGE")){
			...
			if(null != startDate && null != stopDate) {
				/*
				 * Query for any duplicate Special MILEAGE posting dates.
				...
				if(CollectionUtility.hasValue(dupDates)){
					...
					coaVo.setPromptVo(new PromptVo("text.time","action.0140a",new String[]{"Special 'MILEAGE'", "[ "+duplicateDates+" ]"},PromptVo._YES | PromptVo._NO | PromptVo._CANCEL));
