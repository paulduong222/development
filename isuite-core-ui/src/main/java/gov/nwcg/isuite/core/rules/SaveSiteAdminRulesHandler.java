...
import gov.nwcg.isuite.core.rules.user.savesiteadmin.SaveSiteAdminRuleFactory;
...
public class SaveSiteAdminRulesHandler extends AbstractRuleHandler {
	
	public SaveSiteAdminRulesHandler(ApplicationContext ctx) {
		super.context=ctx;
	}
	...
	public int execute(User userEntity, UserVo userVo, DialogueVo dialogueVo) throws Exception {
		...
		try{
			...
			for(SaveSiteAdminRuleFactory.RuleEnum ruleEnum : SaveSiteAdminRuleFactory.RuleEnum.values()){
				IRule rule = SaveSiteAdminRuleFactory.getInstance(ruleEnum, context, userVo, userEntity, userDao);
	...
	public void executeProcessedActions(User userEntity, UserVo userVo, DialogueVo dialogueVo) throws Exception {
