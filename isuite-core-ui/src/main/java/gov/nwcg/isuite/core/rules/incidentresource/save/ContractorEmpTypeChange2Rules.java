...
public class ContractorEmpTypeChange2Rules extends AbstractIncidentResourceSaveRule implements IRule {
	public static final String _RULE_NAME="CONTRACTOR_EMP_TYPE_CHANGE2";

	public ContractorEmpTypeChange2Rules(ApplicationContext ctx){
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		 ...
		 * B.R. 6.0002
		 * 
		 * If the user changes the Contractor/Cooperator designation for a 
		 * Resource assigned to an Incident and that Resource has Time postings 
		 * that were included on an Original Invoice for that Incident, 
		 * the system must not change the Contractor/Cooperator designation until 
		 * the user deletes all of the Resource's Original Invoices for that 
		 * Incident. (Applies to Use Cases 6.0006 and 6.0008.)
		 *           
		 */
