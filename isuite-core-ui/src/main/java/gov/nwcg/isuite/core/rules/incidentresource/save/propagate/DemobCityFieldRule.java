...
public class DemobCityFieldRule extends AbstractPropagateRule {
	public static final String FIELD_NAME="Demob City";
	...
	private PropagateFieldPromptVo checkNonStrikeTeamRules(){
		...
		if(!StringUtility.hasValue(super.origParentResourceData.dmTentativeDemobCity) && StringUtility.hasValue(super.newParentResourceData.dmTentativeDemobCity)){
			for(ResourceData childData : super.childrenResourceData){
				if(!StringUtility.hasValue(childData.dmTentativeDemobCity)){
					// do auto propagate
		...
		if(StringUtility.hasValue(super.origParentResourceData.dmTentativeDemobCity) && StringUtility.hasValue(super.newParentResourceData.dmTentativeDemobCity)){
			if(!super.origParentResourceData.dmTentativeDemobCity.equals(newParentResourceData.dmTentativeDemobCity)){
				// prompt user to propagate
				if(BooleanUtility.isFalse(PropagateFieldPromptVo.hasFieldVo(FIELD_NAME, super.propagateFieldPromptVos))){
					propagateFieldPromptVo.setPromptUserMessage("Demob City" );
