...
public class UnmatchedResourcesNoReqNumRules extends AbstractRossImportRule implements IWizardRule{
	private static final String _RULE_NAME=RossImportProcessRuleFactory.RuleEnum.UNMATCHED_RESOURCES_NO_REQ_NUM.name();

	public UnmatchedResourcesNoReqNumRules(ApplicationContext ctx){
	...
	private Boolean buildResourceUnMatches(DialogueVo dialogueVo) throws Exception{
		...
		for(RossImportProcessResourceVo ripResVo : ripVo.getRossImportProcessResourceVos()){
			if(!ripResVo.getExcludeResource() && 
				...
				if(!UnmatchedResourcesNoReqNumWizardVo.hasVo(ripVo.getUnmatchedResourcesNoReqNumWizardVo().getRossUnmatchedVos(), ripResVo)){
					// is the req num empty
					if(!StringUtility.hasValue(ripResVo.getRequestNumber())){
						// add it
						ripVo.getUnmatchedResourcesNoReqNumWizardVo().getRossUnmatchedVos().add(ripResVo);
		...
		for(RossImportProcessEISuiteResourceVo eisuiteVo : eisuiteResources){
			...
			if(!rossMap.containsKey(eisuiteVo.getResourceId()) &&
					!UnmatchedResourcesNoReqNumWizardVo.hasEisuiteVo(ripVo.getUnmatchedResourcesNoReqNumWizardVo().getEisuiteUnmatchedVos(), eisuiteVo)){
			//	System.out.println("Adding " + eisuiteVo.getResourceId());
				eisuiteVo.setRossResReqId(null);
				ripVo.getUnmatchedResourcesNoReqNumWizardVo().getEisuiteUnmatchedVos().add(eisuiteVo);
				eresvos.add(eisuiteVo);
			}
		}
		
		if(ripVo.getUnmatchedResourcesNoReqNumWizardVo().getRossUnmatchedVos().size()>0){
			// update the rossimportprocessVo in dialogue.resultObject
			if(CollectionUtility.hasValue(ripresvos)){
				((RossImportProcessVo)dialogueVo.getResultObject()).getUnmatchedResourcesNoReqNumWizardVo().setRossUnmatchedVos(ripresvos);
			}
			if(CollectionUtility.hasValue(eresvos)){
				((RossImportProcessVo)dialogueVo.getResultObject()).getUnmatchedResourcesNoReqNumWizardVo().setEisuiteUnmatchedVos(eresvos);
			}
