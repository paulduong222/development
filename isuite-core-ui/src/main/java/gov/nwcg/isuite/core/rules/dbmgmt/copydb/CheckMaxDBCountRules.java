...
public class CheckMaxDBCountRules extends AbstractDbMgmtCopyRule implements IRule {
	public static final String _RULE_NAME = DbMgmtCopyRuleFactory.RuleEnum.CHECK_MAX_DB_COUNT.name();
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		 ...
		 */
		try{
			if(!LongUtility.hasValue(targetVo.getId())){
				...
				if(isExceedMax==true){
					String msg="There can only be five active databases in the e-ISuite System. You must remove an existing database from the system before creating the new database.";
