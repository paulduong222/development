...
public class CheckNapAccountManagerDeleteRules extends AbstractDeleteUserRule implements IRule{
	public static final String _RULE_NAME=DeleteUsersRuleFactory.RuleEnum.CHECK_NAP_ACCOUNT_MANAGER_DELETE_RULE.name();

	public CheckNapAccountManagerDeleteRules(ApplicationContext ctx)
	{
	...
	private int runRuleCheck(DialogueVo dialogueVo) {
		...
		if(runMode.equals("ENTERPRISE")){
					...
					if(sessionUserIsAccountManagerOnly()==true){
						...
						if(cnt>0){
							ErrorObject error2 = new ErrorObject("error.800000","User with only the Account Manager Role cannot delete Data, Global Reference, or Geographic Data Managers.");
