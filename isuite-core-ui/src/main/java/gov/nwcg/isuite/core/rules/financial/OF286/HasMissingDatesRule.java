...
public class HasMissingDatesRule extends AbstractInvoiceGenerationRule implements IRule {
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		if(filter.getPrintInvoiceOnly() || filter.getPrintDeductionsAndInvoice()) {
			...
			for(IncidentResourceVo irvo : super.irs){
				...
				Collection<Date> irDates = tpDao.getNonInvoicedUniqueTimePostDates(irvo.getId());
				...
				if(CollectionUtility.hasValue(irDates)){
					...
					while(!DateUtil.isSameDate(firstDate, lastDate)){
						...
						if(!dateHash.contains(sdate)){
							...
							mpdVo.setIncidentResourceId(irvo.getId());
	...
	public String getMissingDates() {
		...
		for (Date dt : dates) {
			...
			cals.add(cal);
			if (count == 1) {
				firstDate.setTime(cal.getTime());
				lastDate.setTime(cal.getTime());
			}
			if (cal.before(firstDate))
				firstDate.setTime(cal.getTime());
		...
		while (!firstDate.equals(lastDate)) {
			...
			if (!cals.contains(firstDate)) {
				missingDates.add(firstDate.getTime());
			}
		}
		...
		for (Date d : missingDates) {
			if(dateList.length()>1)
				dateList = dateList + ","+ formatter.format(d) ;
			else
				dateList = dateList + formatter.format(d) ;
		}
