...
public class CheckChildGenCostsFlagRules extends AbstractPersonnelRule implements IRule{
	public static final String _RULE_NAME=TimePostPersonnelRuleFactory.RuleEnum.CHECK_CHILD_GEN_COST_FLAG.name();
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		...
		try{
			...
			IncidentResource ir = irdao.getByAssignmentTimeId(vo.getAssignmentTimeId());
			if(null != ir && null != ir.getResource() && (null != ir.getResource().getParentResource() || LongUtility.hasValue(ir.getResource().getParentResourceId()))){
				...
				if(null != ir.getCostData() && BooleanUtility.isFalse(StringBooleanEnum.toBooleanValue(ir.getCostData().getGenerateCostsSys()))){
					...
					courseOfActionVo.setStoredObject(ir.getId());
					courseOfActionVo.setStoredObject1(ir.getResource().getParentResource().getId());
	...
	public void executePostProcessActions(DialogueVo dialogueVo) throws Exception {
		...
		if(null != coaVo && coaVo.getCoaType().equals(CourseOfActionTypeEnum.ADDITIONAL_ACTION_NEEDED)){
			if(null != coaVo.getStoredObject()){
				Long irId=(Long)coaVo.getStoredObject();
				Long parentResourceId=(Long)coaVo.getStoredObject1();
				
				try{
					IncidentResourceDao irDao = (IncidentResourceDao)context.getBean("incidentResourceDao");
					IncidentResource childIncResource=irDao.getById(irId, IncidentResourceImpl.class);
					...
					/*
					childIncResource.getCostData().setGenerateCosts(true);
					childIncResource.getCostData().setGenerateCostsSys(StringBooleanEnum.Y);
					irDao.save(childIncResource);
					irDao.flushAndEvict(childIncResource.getCostData());
					irDao.flushAndEvict(childIncResource);
					*/
				}catch(Exception e){
					//smother
