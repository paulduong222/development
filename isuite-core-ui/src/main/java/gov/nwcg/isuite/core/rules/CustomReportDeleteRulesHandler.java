...
import gov.nwcg.isuite.core.rules.reports.customreports.delete.CustomReportDeleteRuleFactory;
...
public class CustomReportDeleteRulesHandler extends AbstractRuleHandler {
	
	public CustomReportDeleteRulesHandler(ApplicationContext ctx) {
		super.context=ctx;
	}
	...
	public int execute(CustomReportVo filter,
		...
		try{
			for(CustomReportDeleteRuleFactory.RuleEnum ruleEnum : CustomReportDeleteRuleFactory.RuleEnum.values()){
				IRule rule = CustomReportDeleteRuleFactory.getInstance(ruleEnum, context, filter, userVo);
	...
	public void addAdditionalMessages(DialogueVo dialogueVo, UserVo userVo) throws Exception {
		try{
			...
			for(CustomReportDeleteRuleFactory.RuleEnum ruleEnum : CustomReportDeleteRuleFactory.RuleEnum.values()){
				IRule rule = CustomReportDeleteRuleFactory.getInstance(ruleEnum, context, null, null);
