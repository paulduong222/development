...
public class UserAcctDisabledWhenSavedRules extends AbstractSaveUserRule implements IRule {
	
	public static final String _RULE_NAME=SaveUserRuleFactory.RuleEnum.USER_ACCT_DISABLED_WHEN_SAVED_RULE.name();
	
	public UserAcctDisabledWhenSavedRules(ApplicationContext ctx) {
	...
	public void executePostProcessActions(DialogueVo dialogueVo) throws Exception {
		...
		if(coaVo != null && coaVo.getCoaType().equals(CourseOfActionTypeEnum.ADDITIONAL_ACTION_NEEDED)){
			UserSessionManagementService2 usms = (UserSessionManagementService2)context.getBean("userSessionManagementService2");

			Collection<Integer> userIds = new ArrayList<Integer>();
			userIds.add(userVo.getId().intValue());
			
			usms.disconnectUserSessions(userIds, dbName, dialogueVo);
