...
public class CheckMatchIncidentRules extends AbstractRossImportBeginRule implements IRule{
	public static final String _RULE_NAME=RossImportProcessBeginRuleFactory.RuleEnum.CHECK_MATCH_INCIDENT.name();
	
	public CheckMatchIncidentRules(ApplicationContext ctx)
	{
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		...
		if(CollectionUtility.hasValue(vos)){
			...
			rossImportVo.setEisuiteIncidentId(incidentVo.getId());
			rossImportVo.setEisuiteIncidentName(incidentVo.getIncidentName());
			rossImportVo.setEisuiteIncidentNumber(incidentVo.getHomeUnitVo().getUnitCode()+"-"+ incidentVo.getIncidentNumber());
			rossImportVo.setHasIncidentMatch(true);
		}
