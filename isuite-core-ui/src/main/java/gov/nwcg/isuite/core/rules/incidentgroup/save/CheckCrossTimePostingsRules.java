...
public class CheckCrossTimePostingsRules extends AbstractIncidentGroupSaveRule implements IRule{
	public static final String _RULE_NAME = IncidentGroupSaveRuleFactory.RuleEnum.CHECK_CROSS_TIME_POSTINGS.name();
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		/*
		 * 
		 *  4.  When an Incident Group is selected and a user has posted time for an Incident 
		 *  in the Incident Group, the system must not allow the user to remove that Incident from the Incident Group
		 *  
		 *  
		 * Check if incident is being removed from incident group.
		 * If so, verify that incident does have not resources that
		 * have cross incident time postings.
		 */
		if(null != entity){
			...
			for(Incident inc : removeList){
				...
				int cnt = super.incidentGroupDao.getCrossIncidentTimePostings(inc.getId());
				if(cnt > 0){
