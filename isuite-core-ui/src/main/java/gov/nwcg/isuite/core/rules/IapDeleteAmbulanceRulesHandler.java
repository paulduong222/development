...
import gov.nwcg.isuite.core.rules.iap.deleteambulance.DeleteAmbulanceRuleFactory;
...
public class IapDeleteAmbulanceRulesHandler extends AbstractRuleHandler {

	public IapDeleteAmbulanceRulesHandler(ApplicationContext ctx) {
		super.context = ctx;
	}
	...
	public int execute(IapMedicalAidVo vo, DialogueVo dialogueVo) throws Exception {
		...
		try {
			...
			for(DeleteAmbulanceRuleFactory.RuleEnum ruleEnum : DeleteAmbulanceRuleFactory.RuleEnum.values()) {
				IRule rule = DeleteAmbulanceRuleFactory.getInstance(ruleEnum, context, vo);
