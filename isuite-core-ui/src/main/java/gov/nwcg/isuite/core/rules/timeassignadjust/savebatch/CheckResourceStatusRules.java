...
public class CheckResourceStatusRules extends AbstractTimeAssignAdjustSaveBatchRule implements IRule{
	public static final String _RULE_NAME=TimeAssignAdjustSaveBatchRuleFactory.RuleEnum.CHECK_RESOURCE_STATUS.name();
	...
	private int runRuleCheck(DialogueVo dialogueVo) {
		...
		if(null != assignmentEntity){
			if(null != assignmentEntity.getAssignmentStatus()){
				String status = assignmentEntity.getAssignmentStatus().name();
		...
		if(!passed){
			 ...
			 * Build Error message 
			 */
			String errorMsg="You cannot post an adjustment to a Resource that does not have an active status.";
