...
public class CheckRunCostAutoRules extends AbstractCrewRule implements IRule{
	public static final String _RULE_NAME=TimePostCrewRuleFactory.RuleEnum.CHECK_RUN_COST_AUTO.name();
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		...
		IncidentResource irEntity = irDao.getByAssignmentTimeId(atId);
		if(null != irEntity){
		...
		if(null != coaVo && coaVo.getCoaType().equals(CourseOfActionTypeEnum.ADDITIONAL_ACTION_NEEDED)){
			if(null != coaVo.getStoredObject()){
				...
				try{
					...
					for(AssignmentTime at : super.entities){
						IncidentResource irEntity = irDao.getByAssignmentTimeId(at.getId());
