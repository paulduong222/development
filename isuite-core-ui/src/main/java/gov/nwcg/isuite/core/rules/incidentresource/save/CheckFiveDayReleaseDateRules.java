...
public class CheckFiveDayReleaseDateRules extends AbstractIncidentResourceSaveRule implements IRule{
	public static final String _RULE_NAME=IncidentResourceSaveRuleFactory.RuleEnum.CHECK_FIVE_DAY_RELEASE_DATE.name();

	public CheckFiveDayReleaseDateRules(ApplicationContext ctx)
	{
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception{
		...
		/*
		 * CR 129
		 * Add/Edit Resources/Common Data - Actual Release Date
		...
		if(DateUtil.hasValue(releaseDate)){
			...
			if(chkDate.after(sysDate)){
				...
				if((super.irEntity != null)){
					if(DateUtil.hasValue(irEntity.getWorkPeriod().getDMReleaseDate())){
						entityDate=irEntity.getWorkPeriod().getDMReleaseDate();
				...
				if(diffDays2 != 0){
					if(diffDays > 5){
						...
						coaVo.setPromptVo(new PromptVo("text.incidentResources"
											,"info.0299.a"
											,new String[]{"Actual Release Date ("+DateUtil.toDateString(chkDate)+")"}
