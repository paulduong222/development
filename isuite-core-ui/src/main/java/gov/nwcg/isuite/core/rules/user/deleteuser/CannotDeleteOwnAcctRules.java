...
public class CannotDeleteOwnAcctRules extends AbstractDeleteUserRule implements IRule {

	public static final String _RULE_NAME=DeleteUsersRuleFactory.RuleEnum.CANNOT_DELETE_OWN_ACCT_RULE.name();

	public CannotDeleteOwnAcctRules(ApplicationContext ctx) {
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		...
		General Requirements
			...
			9.	The system must prevent a user 
				from deleting one’s own User Account. 

		*/ 
		
	    if(super.getUserSessionVo().getUserId().equals(userEntity.getId())) {
	    	...
	    	courseOfActionVo.setMessageVo(
	    			new MessageVo(
	    					"text.userAccounts",
	    					"error.cannotDeleteYourOwnAccount",
	    					new String[]{userVo.getFirstName() + " " + userVo.getLastName()},
