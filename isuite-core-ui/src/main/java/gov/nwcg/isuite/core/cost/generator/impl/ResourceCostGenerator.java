...
public class ResourceCostGenerator extends AbstractGenerator implements ICostGenerator {
	...
	public void userFlowdown(IncidentResourceDailyCost dailyCostOrigin,IncidentResourceDailyCostVo preSaveVo) throws DailyCostException{
		try{
			 ...
			 * - Identify which fields changed between the new updatedCostRecord and the dailyCostOrigin record
			 * 	 , only flowdown the data that changed.
			 * 
			 */
			for(IncidentResourceDailyCost entity : dailyCosts){
				...
				//if(entity.getCostLevel()!=CostLevelEnum.A){
					 ...
					 *        No, only skip the locked records
					 */
					
					// If not locked and not ManualCostRecord, proceed
					if(BooleanUtility.isFalse(entity.getIsLocked()) 
								&& !entity.getCostLevel().equals("M") && entity.getId().compareTo(dailyCostOrigin.getId())!=0){
	...
	public void generateCosts(Date startDate,Boolean updateRatesOnly) throws Exception {
		...
		try{
			...
			if( (DateUtil.hasValue(dailyCostStartDate) && DateUtil.hasValue(dailyCostEndDate))
				...
				// delete all estimated cost records for different acctcode than resource's default.
				IncidentAccountCodeVo iacvo = irVo.getWorkPeriodVo().getWpDefaultIncidentAccountCodeVo();
				if(null != iacvo && null != iacvo.getId()){
					//irdcDao.deleteResEstCoststByAcctCodeId(irVo.getId(), null, iacvo.getId());
				}

				// if use actuals is true, delete all estimates
				if(null != irVo.getCostDataVo()){
					if(BooleanUtility.isTrue(irVo.getCostDataVo().getUseAccrualsOnly())){
						irdcDao.deleteResEstCosts(irVo.getId(), null);
					}
				}				
				
				loadCollections(CostTypeEnum.RESOURCE);
				...
				dayCalculator.resourceCostType=ResourceCostTypeEnum.RESOURCE;
				...
				//System.out.println("ResName:" + irEntity.getResource().getResourceName());
				//System.out.println("LastName:" + irEntity.getResource().getLastName());
					...
					for(Date d : resourceCostDates){
						...
						if(CollectionUtility.hasValue(costsToSave)){
							for(IncidentResourceDailyCost cost : costsToSave){
								irdcDao.save(cost);
								irdcDao.flushAndEvict(cost.getIncidentAccountCode().getAccountCode());
						...
						if(CollectionUtility.hasValue(irdcEntities)){
							for(IncidentResourceDailyCost cost : irdcEntities){
								try{
									if(null != cost.getIncidentAccountCode() && null != cost.getIncidentAccountCode().getAccountCode())
										irdcDao.flushAndEvict(cost.getIncidentAccountCode().getAccountCode());
