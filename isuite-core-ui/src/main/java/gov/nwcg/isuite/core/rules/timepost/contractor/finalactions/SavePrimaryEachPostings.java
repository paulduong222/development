...
import gov.nwcg.isuite.core.rules.timepost.contractor.ContractorPrimaryEachDuplicateRules;
...
public class SavePrimaryEachPostings extends BaseFinalAction {
	private Boolean needOverwrite=false;
	private Collection<Long> duplicateIds = new ArrayList<Long>();

	public SavePrimaryEachPostings(TimePostDao dao
	...
	public void save(AssignmentTimePost entity,DialogueVo dialogueVo) throws Exception {
		 ...
		 */
		needOverwrite = 
			checkForAdditionalActionNeeded(ContractorPrimaryEachDuplicateRules._RULE_NAME,dialogueVo);

		if(needOverwrite==true){
			CourseOfActionVo coaVo = super.getCoa(ContractorPrimaryEachDuplicateRules._RULE_NAME, dialogueVo);
			if(null != coaVo){
