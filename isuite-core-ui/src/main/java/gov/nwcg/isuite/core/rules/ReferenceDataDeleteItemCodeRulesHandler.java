...
import gov.nwcg.isuite.core.rules.referencedata.itemcodes.delete.ReferenceDataDeleteItemCodeRuleFactory;
...
public class ReferenceDataDeleteItemCodeRulesHandler extends AbstractRuleHandler {
	
	public ReferenceDataDeleteItemCodeRulesHandler(ApplicationContext ctx) {
		super.context=ctx;
	}
	...
	public int execute(KindVo vo, Kind entity, DialogueVo dialogueVo) throws Exception {
		...
		try{
			...
			for(ReferenceDataDeleteItemCodeRuleFactory.RuleEnum ruleEnum : ReferenceDataDeleteItemCodeRuleFactory.RuleEnum.values()){
				IRule rule = ReferenceDataDeleteItemCodeRuleFactory.getInstance(ruleEnum, vo, entity, context, kindDao);
