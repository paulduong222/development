...
public class ContractorSpecialHourlyDuplicateRules extends AbstractContractorRule implements IRule {
	public static final String _RULE_NAME="CONTRACTOR_SPECIAL_HOURLY_DUPLICATE";

	public ContractorSpecialHourlyDuplicateRules(ApplicationContext ctx){
		...
		if((postType.equals("SPECIAL") || postType.equals("BOTH")) 
			...
			if(null != startTime && null != stopTime) {
				...
				if(stopTime.before(startTime)){
					 ...
					 * Query for any duplicate Special Hourly posting dates for the first date
					 */
					...
					// is replacing them
					if(LongUtility.hasValue(super.specialVo.getId())){
						Collection<Long> newids = new ArrayList<Long>();
						newids.add(specialVo.getId());
					...
					/*
					 * Query for any duplicate Special Hourly posting dates.
					...
					dupIds= 
						tpDao.getDuplicateDailyPostsIds(tpqFilter);
				...
				if(CollectionUtility.hasValue(dupDates)){
					...
					coaVo.setPromptVo(new PromptVo("text.time","action.0140b2",new String[]{"Special 'HOURLY'", "[ "+duplicateDates+" ]",dupTimes},PromptVo._YES | PromptVo._NO | PromptVo._CANCEL));
