...
public class DeleteUserEventRules extends AbstractDeleteUserRule implements IRule{
	public static final String _RULE_NAME=DeleteUsersRuleFactory.RuleEnum.DELETE_USER_EVENT.name();

	public DeleteUserEventRules(ApplicationContext ctx)
	{
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception{
		...
		if(runMode.equals("SITE")){
			...
			try{
				dac=dacDao.getByEventName("ISW_USER", DataAuditEvent.ACCOUNT_DELETED.name());
			}catch(Exception e){
				//smother
			}
			...
			if(null != dac && dac.getEnabled()==StringBooleanEnum.Y ){
				...
				courseOfActionVo.setStoredObject("USER " + super.userVo.getLoginName() + " DELETED");
	...
	public void executePostProcessActions(DialogueVo dialogueVo) throws Exception {
		...
		if(null != coaVo && coaVo.getCoaType().equals(CourseOfActionTypeEnum.ADDITIONAL_ACTION_NEEDED)){
			if(null != coaVo.getStoredObject()){
				...
				String name=(String)coaVo.getStoredObject();
				...
				dataAuditTracking.setOldValue("ACCOUNT ACTIVE");
				dataAuditTracking.setNewValue("ACCOUNT DELETED");
