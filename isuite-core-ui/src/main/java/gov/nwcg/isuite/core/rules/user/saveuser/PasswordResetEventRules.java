...
public class PasswordResetEventRules extends AbstractSaveUserRule implements IRule{
	public static final String _RULE_NAME=SaveUserRuleFactory.RuleEnum.PASSWORD_RESET_EVENT.name();
			...
			if(null != dac && dac.getEnabled()==StringBooleanEnum.Y ){
				if(null!=userEntity && LongUtility.hasValue(super.userEntity.getId())){
					if(!userVo.getPassword().equals(UserVo.DISPLAY_PASSWORD)){
						if(userEntity.getId() == super.getUserSessionVo().getUserId())
							passwordChangedByWho = "BY USER";
						else
							passwordChangedByWho = "BY MANAGER";
