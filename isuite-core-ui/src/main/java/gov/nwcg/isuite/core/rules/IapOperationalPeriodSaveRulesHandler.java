...
import gov.nwcg.isuite.core.rules.iap.saveoperationalperiod.OperationalPeriodSaveRuleFactory;
...
public class IapOperationalPeriodSaveRulesHandler extends AbstractRuleHandler {

	public IapOperationalPeriodSaveRulesHandler(ApplicationContext ctx) {
		super.context = ctx;
	}
	...
	public int execute(IncidentShiftVo incidentShiftVo, IncidentShift entity, DialogueVo dialogueVo) throws Exception {
		...
		try {
			...
			for(OperationalPeriodSaveRuleFactory.RuleEnum ruleEnum : OperationalPeriodSaveRuleFactory.RuleEnum.values()) {
				IRule rule = OperationalPeriodSaveRuleFactory.getInstance(
