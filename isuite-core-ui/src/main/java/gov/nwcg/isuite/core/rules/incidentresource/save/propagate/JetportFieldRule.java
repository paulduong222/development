...
public class JetportFieldRule extends AbstractPropagateRule {
	public static final String FIELD_NAME="Jetport";
	...
	private PropagateFieldPromptVo checkNonStrikeTeamRules(){
		...
		if(!LongUtility.hasValue(super.origParentResourceData.ciArrivalJetPortId) && LongUtility.hasValue(super.newParentResourceData.ciArrivalJetPortId)){
			for(ResourceData childData : super.childrenResourceData){
				if(!LongUtility.hasValue(childData.ciArrivalJetPortId)){
					// do auto propagate
		...
		if(LongUtility.hasValue(super.origParentResourceData.ciArrivalJetPortId) && LongUtility.hasValue(super.newParentResourceData.ciArrivalJetPortId)){
			if(super.origParentResourceData.ciArrivalJetPortId.compareTo(newParentResourceData.ciArrivalJetPortId)!=0){
				// prompt user to propagate
				if(BooleanUtility.isFalse(PropagateFieldPromptVo.hasFieldVo(FIELD_NAME, super.propagateFieldPromptVos))){
					propagateFieldPromptVo.setPromptUserMessage("Jetport" );
