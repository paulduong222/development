...
public class CheckLockedByContractorIdRules extends AbstractSiteManagedRule implements IRule{
	public static final String _RULE_NAME=SiteManagedRuleFactory.RuleEnum.IS_INCIDENT_LOCKED_BY_CONTRACTOR_ID.name();
	
	public CheckLockedByContractorIdRules(ApplicationContext ctx)
	{
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		if(LongUtility.hasValue(super.id) 
				&& super.idType.equals("CONTRACTOR")
			...
			// get incident id
			Long incidentId=0L;
			incidentId=cdao.getIncidentId(super.id);
