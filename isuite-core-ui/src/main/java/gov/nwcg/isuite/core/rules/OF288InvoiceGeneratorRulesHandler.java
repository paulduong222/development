...
import gov.nwcg.isuite.core.rules.financial.OF288.OF288InvoiceGeneratorRuleFactory;
...
public class OF288InvoiceGeneratorRulesHandler extends AbstractRuleHandler {
	
	public OF288InvoiceGeneratorRulesHandler(ApplicationContext ctx) {
		super.context=ctx;
	}
	...
	public int execute(Collection<IncidentResourceVo> irs,
		...
		try{
			for(OF288InvoiceGeneratorRuleFactory.RuleEnum ruleEnum : OF288InvoiceGeneratorRuleFactory.RuleEnum.values()){
				IRule rule = OF288InvoiceGeneratorRuleFactory.getInstance(ruleEnum, context, irs, dates, filter);
	...
	public void addAdditionalMessages(DialogueVo dialogueVo) throws Exception {
		try{
			...
			for(OF288InvoiceGeneratorRuleFactory.RuleEnum ruleEnum : OF288InvoiceGeneratorRuleFactory.RuleEnum.values()){
				IRule rule = OF288InvoiceGeneratorRuleFactory.getInstance(ruleEnum, context, null, null, null);
