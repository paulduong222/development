		...
		try{
			...
			if(super.getRunMode().equals("ENTERPRISE")){
				...
				if(CollectionUtility.hasValue(super.dataTransferXml.getIncidents())){
					Collection<IswIncident> incidents = super.dataTransferXml.getIncidents();
					
					for(IswIncident inc : incidents){
						String ti=inc.getTransferableIdentity();
						
						Long existingIncidentId=incDao.getIncidentIdByTi(ti);
						
						if(LongUtility.hasValue(existingIncidentId)){
							String incomingName=inc.getIncidentName();
