...
public class ValidateDataRules extends AbstractCustomReportGenerationRule implements IRule {
	...
	private int runRuleCheck(DialogueVo dialogueVo) throws Exception {
		...
		error = Validator.validateVoField2("View", reportVo.getCustomReportViewVo(), true);
		if(null != error)errorObjects.add(error);
		
		//check for report title
		error = Validator.validateStringField2("Report Title", reportVo.getReportTitle(), 50, true);
		if(null != error)errorObjects.add(error);
