...
import gov.nwcg.isuite.core.rules.timepost.contractor.ContractorSpecialHourlyOverlapRules;
...
public class SaveSpecialHourlyPostings extends BaseFinalAction {
	...
	public SaveSpecialHourlyPostings(TimePostDao dao
	...
	public void save(AssignmentTimePost entity,DialogueVo dialogueVo) throws Exception {
		 ...
		 */
		needSplitPosting = 
			checkForAdditionalActionNeeded(ContractorSpecialHourlyOverlapRules._RULE_NAME,dialogueVo);
		
		/*
		 * Set flag if we need to overwrite existing records.
		 */
		needOverwrite = 
			checkForAdditionalActionNeeded(ContractorSpecialHourlyDuplicateRules._RULE_NAME,dialogueVo);
		
		/*
		 * Get the original start and stop times
		 */
		originalStartTime=specialVo.getPostStartTime();
		originalStopTime=specialVo.getPostStopTime();
		...
		if(LongUtility.hasValue(specialVo.getId()))
			filter.setExcludeAssignmentTimePostId(specialVo.getId());
		filter.setIncidentResourceId(incidentResourceEntity.getId());
		filter.setOnlyPrimary(false);
		filter.setOnlySpecial(true);
		filter.setExcludeGuarantee(true);
		filter.setCompareTime(true);
		filter.setIncludeTime(true);
		filter.setPostType("SPECIAL");
		...
		if(null != entity){
			...
			if(needSplitPosting){
				...
				if(needOverwrite){
					...
					CourseOfActionVo coa = super.getCoa(ContractorSpecialHourlyDuplicateRules._RULE_NAME,dialogueVo);
					...
					for(Object id : ids){
						...
						if(i>0){
							...
							AssignmentTimePost atp = tpDao.getById(lngId, AssignmentTimePostImpl.class);
							tpDao.flushAndEvict(atp);
							Date start = DateUtil.addMilitaryTimeToDate(specialVo.getPostStartDate(), specialVo.getPostStartTime());
							atp.setPostStartDate(start);
							Date stop = DateUtil.addMilitaryTimeToDate(specialVo.getPostStopDate(), specialVo.getPostStopTime());
					...
					atpvo.setUnitOfMeasure(specialVo.getRefContractorRateVo().getUnitOfMeasure());
					atpvo.setRateAmount(specialVo.getRefContractorRateVo().getRateAmount());
					atpvo.setPostStartDate(DateUtil.addMilitaryTimeToDate(specialVo.getPostStartDate(), originalStartTime));
					atpvo.setPostStopDate(DateUtil.addMilitaryTimeToDate(specialVo.getPostStartDate(), originalStopTime));
					if(null != specialVo.getIncidentAccountCodeVo()){
						IncidentAccountCode iac = new IncidentAccountCodeImpl();
						iac.setId(specialVo.getIncidentAccountCodeVo().getId());
						iac.setAccountCode(AccountCodeVo.toEntity(null, specialVo.getIncidentAccountCodeVo().getAccountCodeVo(), true));
				...
				if(needOverwrite){
					...
					for(Object id : ids){
						...
						if(i>0){
							...
							atp.setPostStartDate(start);
							Date stop =null;
							if(null != specialVo.getPostStopDate())
								stop = DateUtil.addMilitaryTimeToDate(specialVo.getPostStopDate(), specialVo.getPostStopTime());
							else
								stop = DateUtil.addMilitaryTimeToDate(specialVo.getPostStartDate(), specialVo.getPostStopTime());
							atp.setPostStopDate(stop);
	...
	private void splitTimePosting(DialogueVo dialogueVo) throws Exception {
		 ...
		 */
		AssignmentTimePostVo vo1=splitTime(0, specialVo, "2359");
		...
		if(needOverwrite){
			...
			CourseOfActionVo coa =dialogueVo.getCourseOfActionByName(ContractorSpecialHourlyDuplicateRules._RULE_NAME);
			...
			if(LongUtility.hasValue(specialVo.getId())){
				AssignmentTimePost entity =tpDao.getById(specialVo.getId(), AssignmentTimePostImpl.class);
		...
		AssignmentTimePostVo vo2=splitTime(1, specialVo, originalStopTime);
	...
	private void saveHourlyTimePosting(AssignmentTimePost atpEntity,AssignmentTimePostVo atpvo, DialogueVo dialogueVo) throws Exception {
		...
		if(needOverwrite){
			...
			if(null != atpEntity){
				...
				atpEntity.setQuantity(specialVo.getQuantity());
				atpEntity.setIsHalfRate(specialVo.getIsHalfRate());
				if(null != specialVo.getIncidentAccountCodeVo()){
					IncidentAccountCode iac = new IncidentAccountCodeImpl();
					iac.setId(specialVo.getIncidentAccountCodeVo().getId());
					iac.setAccountCode(AccountCodeVo.toEntity(null, specialVo.getIncidentAccountCodeVo().getAccountCodeVo(), true));
				...
				atpEntity = AssignmentTimePostVo.toEntity(atpEntity, atpvo, true);
				atpEntity.setContractorPostType("SPECIAL");
				if(null != specialVo.getIncidentAccountCodeVo()){
					IncidentAccountCode iac = new IncidentAccountCodeImpl();
					iac.setId(specialVo.getIncidentAccountCodeVo().getId());
					iac.setAccountCode(AccountCodeVo.toEntity(null, specialVo.getIncidentAccountCodeVo().getAccountCodeVo(), true));
